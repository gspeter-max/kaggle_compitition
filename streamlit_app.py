...
# --- Custom CSS for a Simple, High-Contrast Dark Mode with Visible Labels ---
st.markdown("""
    <style>
        ...
        /* WIDGET LABEL FIX: Make input labels light and readable */
        div[data-testid="stWidgetLabel"] label p { /* Targets the <p> inside the label for st.selectbox, st.number_input etc. */
            color: #D0D0D0 !important; /* Lighter grey for labels */
            font-size: 0.95em !important; /* Optional: adjust size */
        }
        /* You might also need to target labels for st.multiselect specifically if they behave differently */
        /* For st.multiselect, the label is often outside the stWidgetLabel div, or structured differently */
        /* This is a more general approach that might catch it, or inspect element for specific class */
        .stMultiSelect div[data-baseweb="block"] > div:first-child { /* Attempt to target multiselect label container */
        ...
        .stMultiSelect > div > div > div > div { /* This styles the input part of multiselect */
...
if not st.session_state.show_results and model is not None:
    with st.form(key="sales_input_form"):
        ...
        with row1_col1:
            st.markdown("##### 📅 Date & Time") # Markdown headers will use general h5 color
        ...
        st.markdown("---")
        # The label for multiselect is directly part of its arguments
        promo_interval_header_order = ['Jan', 'Apr', 'Jul', 'Oct', 'Feb', 'May', 'Aug', 'Nov', 'Mar', 'Jun', 'Sept', 'Dec']
        promo_interval_active_months = st.multiselect("🚀 Promo2 Interval Months (if Promo2 is active):", 
                                                      promo_interval_header_order,
        ...
        def create_feature_vector(inputs_dict):
            ...
            for month_name in inputs_dict['promo_interval_header_order']: # Use the list as defined in the form scope
        ...
        if submitted:
            current_inputs = {
                ...
                'promo_interval_header_order': promo_interval_header_order, # Pass the order for feature creation
            ...
            with st.spinner("🧠 Calculating forecast..."):
                try:
                    ...
                    if feature_vector.shape[1] != EXPECTED_NUM_FEATURES:
                        st.error(f"Feature Mismatch Error. Expected {EXPECTED_NUM_FEATURES}, got {feature_vector.shape[1]}. Please check inputs or model expectations, especially the 'StateHoliday' logic.")
                    else:
                        prediction = model.predict(feature_vector)
                        predicted_sales = np.expm1(prediction[0])
                        st.session_state.predicted_sales_value = predicted_sales
                        st.session_state.show_results = True
                        st.rerun() # Use st.rerun for cleaner state update and UI refresh
                except Exception as e:
